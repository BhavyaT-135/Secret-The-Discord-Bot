...
def remove_points(username_id, points):
    ...
    cur.execute("UPDATE users SET points = points - ? WHERE username = ?", t)
    cur.execute(
        "UPDATE users SET points = 0 WHERE username = ? AND points < 0", t2)
    conn.commit()
...
def add_leaderboard(username, message_id, count):
    ...
    cur.execute(
        "INSERT INTO board_tables(username, message_id, created_time, page_number, last_usernumber) VALUES(?,?,?,?,?)", t)
    conn.commit()
...
def check_leaderboard(message_id, user_id):
    ...
    cur.execute(
        "SELECT COUNT(*) FROM board_tables WHERE username = ? AND message_id = ?", t)
    data = cur.fetchall()
    if(data[0][0] == 0):
        return False
    else:
...
def get_leaderboard_page(message_id, user_id):
    ...
    t = (user_id, message_id, )
    cur.execute(
        "SELECT * FROM board_tables WHERE username = ? AND message_id = ?", t)
    data = cur.fetchall()
...
def update_leaderboard(page, last_user, message_id):
    ...
    cur.execute(
        "UPDATE board_tables SET page_number = ? , last_usernumber = ? WHERE message_id = ?", t)
    conn.commit()
...
def get_users(page=1):
    ...
    conn, cur = get_conn()
    cur.execute("SELECT * FROM users ORDER BY points DESC LIMIT " +
                str(page_offset) + ",10")
    rows = cur.fetchall()
    return rows
...
def insert_points_requests(message_id, users, points, approved, created_by):
    ...
    cur.execute(
        "INSERT INTO points_requests(message_id, users, points, approved, created_by)  VALUES(?,?,?,?,?)", t)
    conn.commit()
...
def check_requests(message_id):
    ...
    t = (message_id, )
    cur.execute(
        "SELECT * FROM points_requests WHERE message_id = ? AND approved = 0 LIMIT 1", t)
    data = cur.fetchall()
    if(not data):
        return None
    else:
...
def update_requests(message_id, app):
    ...
    t = (app, message_id, )
    cur.execute(
        "UPDATE points_requests SET approved = ? WHERE message_id = ?", t)
    conn.commit()
